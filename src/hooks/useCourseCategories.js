...
import courseCategoriesFallback from '../data/courseCategories.json';
...
export function useCourseCategories() {
  ...
  const fetchData = useCallback(async () => {
    ...
    setError(null);
    try {
      const result = await get(ENDPOINTS.CATEGORIES_COURSES);
      ...
      const fallback = Array.isArray(courseCategoriesFallback) ? courseCategoriesFallback : [];
      cache.data = fallback;
      cache.loading = false;
      cache.error = null;
      setData(fallback);
