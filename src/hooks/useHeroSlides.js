...
import heroSlidesFallback from '../data/heroSlides.json';
...
export function useHeroSlides() {
  ...
  const fetchData = useCallback(async () => {
    ...
    setError(null);
    try {
      const result = await get(ENDPOINTS.HERO_SLIDES);
      const list = Array.isArray(result) ? result : result?.data ?? result?.slides ?? [];
      cache.data = list;
      cache.loading = false;
      cache.error = null;
      setData(list);
      ...
      const fallback = Array.isArray(heroSlidesFallback) ? heroSlidesFallback : [];
      cache.data = fallback;
      cache.loading = false;
      cache.error = null;
      setData(fallback);
