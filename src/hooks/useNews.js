...
export function useNews() {
  ...
  const fetchData = useCallback(async () => {
    ...
    setError(null);
    try {
      const result = await get(ENDPOINTS.NEWS);
      const list = Array.isArray(result) ? result : result?.data ?? result?.news ?? [];
      cache.data = list;
      cache.loading = false;
      cache.error = null;
      setData(list);
      ...
      const fallback = Array.isArray(newsFallback) ? newsFallback : [];
      cache.data = fallback;
      cache.loading = false;
      cache.error = null;
      setData(fallback);
