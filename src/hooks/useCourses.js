...
import coursesDataFallback from '../data/courses.json';
...
export function useCourses() {
  ...
  const fetchData = useCallback(async () => {
    ...
    setError(null);
    try {
      const result = await get(ENDPOINTS.COURSES);
      const list = Array.isArray(result) ? result : result?.data ?? result?.courses ?? [];
      cache.data = list;
      cache.loading = false;
      cache.error = null;
      setData(list);
      ...
      const fallback = Array.isArray(coursesDataFallback) ? coursesDataFallback : [];
      cache.data = fallback;
      cache.loading = false;
      cache.error = null;
      setData(fallback);
