 ...
 */
const ErrorDialog = ({ isOpen, onClose }) => {
  ...
  // Focus management and scroll lock - BLOCKING MODE
  useEffect(() => {
    if (isOpen) {
      // Lock body scroll
      ...
      if (modalRef.current) {
        setTimeout(() => {
          const firstFocusable = modalRef.current?.querySelector('button, [href], a');
          if (firstFocusable) {
      ...
      // Trap focus within modal and prevent all other interactions
      ...
      // Prevent all clicks outside modal
  ...
  return (
    <div
      className="error-dialog-container fixed inset-0 z-[9999] flex items-center justify-center p-4"
